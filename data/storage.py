...
class PromoDataManager:
    ...
    def get_promo(self, promo_code: str) -> Dict[str, Any]:
        """Get a specific promotion by code - tries database first, then JSON fallback"""
        # Try database first
        if self.db_manager:
            try:
                ...
                print(f"Database query failed for {promo_code}: {e}")
    ...
    def get_all_promos(self) -> Dict[str, Any]:
        """Get all promotions - tries database first, then JSON fallback"""
        # Try database first
        if self.db_manager:
            try:
                db_records = self.db_manager.get_all_promos()
                if db_records:
                    # Convert list of database records to dict format expected by templates
                    result = {}
                    for record in db_records:
                        converted = self.db_manager.convert_db_record_to_json_format(record)
                        promo_code = converted.get('code', '')
                        if promo_code:
                            result[promo_code] = converted
                    return result
            except Exception as e:
                print(f"Database query failed for all promos: {e}")
