...
def promotions():
    ...
    # Convert to the format expected by the template and filter for RDC
    promotions_list = []
    for promo_code, promo_data in all_promotions.items():
        # Only include promotions where Desired_Execution is 'RDC' or not specified (default to RDC)
        desired_execution = promo_data.get('Desired_Execution', 'RDC')
        if desired_execution == 'RDC':
    ...
    # Get unique owners for filter dropdown (only from RDC promotions)
    owners = ["All"] + list(set([p.get('owner', '') for p in promotions_list if p.get('owner')]))
    
    return render_template(
        "promotions.html",
        promotion_code="RDC Promotions",
        user_name="Cade Holtzen",
        owners=owners,
        selected_owner="All",
        search_query="",
...
def spe():
    ...
    # Convert to the format expected by the template and filter for SPE
    promotions_list = []
    for promo_code, promo_data in all_promotions.items():
        # Only include promotions where Desired_Execution is 'SPE'
        desired_execution = promo_data.get('Desired_Execution', 'RDC')
        if desired_execution == 'SPE':
            promotions_list.append({
                "code": promo_code,
                "orbit_id": promo_data.get('orbit_id', ''),
                "status": "In Progress",  # You can add logic to determine status
    ...
    # Get unique owners for filter dropdown (only from SPE promotions)
    owners = ["All"] + list(set([p.get('owner', '') for p in promotions_list if p.get('owner')]))
