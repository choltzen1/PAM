    ...
    try:
        ...
        if not orbit_row:
            ...
            if legacy_row:
                ...
                err = orbit_err or 'not found'
                status = 404 if 'not found' in str(err).lower() else 500
                return jsonify({'success': False, 'error': f'Orbit {orbit_id} not found' if status == 404 else str(err)}), status
        # Carry forward all normalized Orbit fields except staging metadata.
